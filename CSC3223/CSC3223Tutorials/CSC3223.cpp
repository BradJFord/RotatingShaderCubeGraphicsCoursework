...
void Tutorial12(Renderer* renderer) {
	std::vector <Vector3> verts = { Vector3(-1.0f , -1.0f , 0.0f) , Vector3(-1.0f , 1.0f , 0.0f) ,
	...
	renderer->SetViewMatrix( Matrix4::Rotation(70.0f, Vector3(1, 0, 0)) * Matrix4::Rotation(-225.0f, Vector3(0, 1, 0)) *
	Matrix4::Translation(Vector3(20, -100, 20)));}

RenderObject* Tutorial13(Renderer* renderer) {
	OGLShader* lightingShader = new OGLShader("LightingVertex.glsl",
	...
	renderer->SetLightProperties(Vector3(0, 25, 0),
...
int main() {
	...
	Matrix4 proj = Matrix4::Perspective(1.0f, 120.0f, aspect, 45.0f);
	...
	while (w->UpdateWindow() && !Window::GetKeyboard()->KeyDown(KEYBOARD_ESCAPE)) {
		...
		if (Window::GetKeyboard()->KeyDown(KEYBOARD_S)) {
			...
			renderer->WriteDepthBuffer("Depth.png");		}		//commented out for tutorial 12 vvvvv
